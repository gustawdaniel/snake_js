...
import {Coordinate, GameLog, Direction} from "../types.ts";
...
export default class Snake {
    ...
    inGame: boolean;
    age: number;
    initialConfig: { body: Coordinate[], direction: Direction };
    logs: GameLog[];

    constructor(index: number,body: Coordinate[],direction: Direction) {
        this.index = index;
        this.points = 0;
        this.body = body;
    ...
    containsCoordinates(inspected: Coordinate) {
        return this.body.filter(function (part: Coordinate) {
    ...
    move(direction: Direction) {
        ...
        if (Board.outOfExtendedMap(head) || this.inGame && (Board.outOfMap(head) || this.containsCoordinates(head))) {
            ...
            if (!this.eatApple()) {
                ...
                if(mapCoordinates) {
                    document.querySelector(`div.rect[data-x="${mapCoordinates.x}"][data-y="${mapCoordinates.y}"]`)?.classList.remove(`snake-${this.index}`);
                }
            }
        }
    }
